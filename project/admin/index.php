...
try {
    ...
    $registrationsRaw = $db->query('SELECT * FROM `scheme_registrations` ORDER BY `id` DESC')->fetchAll();
    $registrations = [];
...
<?php else: ?>
    <main class="admin-shell">
        ...
        <!-- Gold Scheme Registrations -->
        <section class="panel">
            <h2><i class="fa-solid fa-users"></i> Gold Savings Scheme Registrations</h2>
            <div class="table-wrap">
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        <?php if (empty($registrations)): ?>
                            <tr>
                                <td colspan="6" style="text-align: center; color: var(--text-muted); padding: 30px;">
                                    <i class="fa-solid fa-file-invoice" style="font-size: 32px; margin-bottom: 12px;"></i>
                                    <p>No gold scheme registrations recorded yet.</p>
                                </td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($registrations as $r): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge active" style="background: rgba(15, 96, 132, 0.1); color: var(--teal-dark);">
                                            <?= e($r['schemeName'] ?? '') ?>
                                        </span>
                                    </td>
                                    <td><strong>₹<?= number_format($r['amount'], 2) ?></strong></td>
                                    <td>
                                        <i class="fa-solid fa-phone" style="color: var(--teal-light); margin-right: 4px;"></i> <?= e($r['phone'] ?? '') ?><br>
                                        <i class="fa-solid fa-envelope" style="color: var(--teal-light); margin-right: 4px;"></i> <a href="mailto:<?= e($r['email'] ?? '') ?>" style="color: inherit; text-decoration: none;"><?= e($r['email'] ?? '') ?></a>
                                    </td>
                                    <td>
                                        <span><?= e($r['address'] ?? '') ?>, <?= e($r['city'] ?? '') ?> - <?= e($r['pincode'] ?? '') ?></span>
                                    </td>
                                    <td><?= date('d M Y, h:i A', strtotime($r['createdAt'])) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
